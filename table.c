...
// Free hash tables just like dynamic arrays
    ...
    for (;;) {
        ...
        if (entry->key == NULL) {
            if (IS_NIL(entry->value)) {
                // Empty entry; return tombstone if found earlier, else entry
...
static void adjustCapacity(Table* table, int capacity) {
    ...
    // Rebuild table using new capacity for key-value insertions
    table->count = 0;
...
bool tableDelete(Table* table, ObjString* key) {
    ...
    // Place a tombstone where the entry was
    entry->key = NULL;
...
void tableAddAll(Table* from, Table* to) {
    // transfer all entries from `from` to `to`
...
ObjString* tableFindString(Table* table, const char* chars, int length, 
    ...
    for (;;) {
        ...
        if (entry->key == NULL) {
            if (IS_NIL(entry->value)) return NULL; // Empty entry; stop
